...
- [`httr`](https://cran.r-project.org/web/packages/cowplot/index.html): extra functionality for `ggplot2`
- [`rmarkdown`](https://cran.r-project.org/web/packages/imager/): loading in images
- [`lubridate`](https://cran.r-project.org/web/packages/broom/vignettes/broom.html):
tidying up a regression output for display
- [`countrycode`](https://cran.r-project.org/web/packages/knitr/index.html): displaying tables in a markdown friendly way
- [`rworldmap`](https://cran.r-project.org/web/packages/imager/): loading in images
...
## `Average`
Calculate the average new case number in the past n days from the day of first case until today. The default n is 
```{r f4, echo=FALSE}
...
```
draw a world heat map of mortality rate 
```{r echo=FALSE}
joinData <- joinCountryData2Map( total_all_pop ,
                                 joinCode = "ISO2",
                                 nameJoinColumn = "ISO2")
...
```
Distribution of total confirmed cases per 1000 people for each country
```{r echo=FALSE}
p2 <- ggplot(total_all_pop, aes(x=cases_per1k)) + 
  geom_histogram()
p2
```
compare new cases per day in a given period (2021-08-1 to 2021-08-30) among different countries
```{r echo=FALSE}
start="2021-08-01"
end="2021-08-30"
...
p3
```
scatter plot of new cases over a period with 7-day average smooth line in one country
```{r echo=FALSE}
...
I found some unsurprising things, like shots per game and shooting percentage are related to win percentage. I also found some surprising (to me) things, namely penalty minutes per game has a quadratic relationship with win percentage.
